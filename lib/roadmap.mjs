...
export function validateRoadmap(data, source = "roadmap") {
  ...
  if (data.schemaVersion !== 1) errors.push(`${source}.schemaVersion must be 1`);
  validateText(errors, data.title, `${source}.title`);
  validateText(errors, data.summary, `${source}.summary`);
  if (!Array.isArray(data.phases) || !data.phases.length) errors.push(`${source}.phases must contain at least one phase`);
  if (!Array.isArray(data.lanes) || !data.lanes.length) errors.push(`${source}.lanes must contain at least one lane`);
  const ids = new Set();
  ...
  for (const [laneIndex, lane] of (data.lanes || []).entries()) {
...
<section class="pipeline-wrap"><div class="pipeline">${phaseCards(data.phases)}</div></section>
...
<section class="lanes">${laneCards(data.lanes)}</section><section class="panel ledger" id="ledger"><div class="kicker">Evidence trail</div><h2>Commit ledger</h2><div class="table-wrap"><table><thead><tr><th>Scope</th><th>Status</th><th>Commit</th><th>Outcome</th><th>Evidence</th></tr></thead><tbody>${ledgerRows(data.ledger, data.lanes)}</tbody></table></div>${data.note ? `<p class="note">${escapeHtml(data.note)}</p>` : ""}</section><footer><span>Assistant Workspace · reusable roadmap artifact v1</span><span>${escapeHtml(data.footer || "Update structured roadmap JSON as implementation evidence lands.")}</span></footer></main><script>
